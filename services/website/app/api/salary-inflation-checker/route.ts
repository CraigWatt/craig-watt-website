  ...
  for (let attempt = 0; attempt <= RETRIES; attempt += 1) {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        'user-agent': 'craigwatt-salary-inflation-checker-dev/1.0',
      },
    });
    ...
    throw new Error(`Fetch failed for ${url} with HTTP ${res.status}${summary ? `: ${summary}` : ''}`);
  }
...
function buildStaticFallbackPayload(): SalaryInflationCheckerPayload {
...
export async function GET() {
  ...
  if (cachedPayload && now - lastFetchedAt < TTL_MS) {
    return response({
      ...cachedPayload,
      _meta: buildMeta(cachedPayload.sourceStatus, {
        cold: false,
        stale: false,
        local: true,
      }),
  ...
  try {
    ...
    console.error('Local salary inflation checker route failed', error);
