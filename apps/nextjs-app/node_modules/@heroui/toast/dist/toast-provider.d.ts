import * as react_jsx_runtime from 'react/jsx-runtime';
import { ToastQueue, ToastOptions } from '@react-stately/toast';
import { ToastProps, ToastPlacement } from './use-toast.js';
import 'tailwind-variants';
import 'react';
import '@heroui/system';
import '@heroui/theme';
import '@heroui/react-utils';
import '@react-aria/toast';
import 'framer-motion';

interface ToastProviderProps {
    maxVisibleToasts?: number;
    placement?: ToastPlacement;
    disableAnimation?: boolean;
    toastProps?: ToastProps;
    toastOffset?: number;
}
declare const getToastQueue: () => ToastQueue<ToastProps>;
declare const ToastProvider: ({ placement, disableAnimation: disableAnimationProp, maxVisibleToasts, toastOffset, toastProps, }: ToastProviderProps) => react_jsx_runtime.JSX.Element | null;
declare const addToast: ({ ...props }: ToastProps & ToastOptions) => void;
declare const closeAll: () => void;

export { ToastProvider, addToast, closeAll, getToastQueue };
