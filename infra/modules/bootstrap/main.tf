...
resource "aws_iam_role" "github_actions" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = { "${local.provider_id}:aud" = "sts.amazonaws.com" }
        StringLike   = { "${local.provider_id}:sub" = "repo:${var.github_repo}:ref:refs/tags/v*" }
      }
    }]
  })
}

# Attach every policy the CI needs:
